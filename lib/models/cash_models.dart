...
DateTime? _parseDate(dynamic value) {
  ...
  if (value is String && value.isNotEmpty) {
    // Gestion robuste des formats de date (avec ou sans 'T')
...
// --- MODIFICATION : Ajout des champs de détail du CA ---
class CashMetrics {
  final double montantEnCaisse;
  
  // Nouveaux champs pour le détail du CA
  final double chiffreAffaireCa; 
  final double caFraisLivraison;
  final double caFraisEmballage;
  final double caFraisStockage;
  ...
  final double creancesNonRemboursees; // "creances_non_remboursees"
  final double manquantsRembourses;    // "manquants_rembourses"
  final double manquantsNonRembourses; // "manquants_non_remboursees"

  CashMetrics({
    this.montantEnCaisse = 0, 
    // Initialisation des nouveaux champs
    this.chiffreAffaireCa = 0,
    this.caFraisLivraison = 0,
    this.caFraisEmballage = 0,
    this.caFraisStockage = 0,
  ...
  factory CashMetrics.fromJson(Map<String, dynamic> json) {
    return CashMetrics(
      ...
      // Mapping des nouveaux champs depuis le JSON du backend
      chiffreAffaireCa: _parseDouble(json['chiffre_affaire_ca']),
      caFraisLivraison: _parseDouble(json['ca_frais_livraison']),
      caFraisEmballage: _parseDouble(json['ca_frais_emballage']),
      caFraisStockage: _parseDouble(json['ca_frais_stockage']),
