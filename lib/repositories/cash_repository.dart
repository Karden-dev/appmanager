...
class CashRepository {
  ...
  Future<List<CashTransaction>> fetchTransactions({
    ...
    return await _apiService.fetchTransactions(
      ...
      endDate: endDate,
      type: type,
      search: search, // <-- PASSAGE AU SERVICE
    );
  }
  ...
  Future<List<Shortfall>> fetchShortfalls({String? status, String? search}) async {
    return await _apiService.fetchShortfalls(status: status, search: search);
  }
  ...
  Future<List<RemittanceSummaryItem>> fetchRemittanceSummary({
    required DateTime startDate, 
    required DateTime endDate,
    String? search, // Optionnel, déjà supporté par le service si besoin
  }) async {
    return await _apiService.fetchRemittanceSummary(startDate: startDate, endDate: endDate, search: search);
  }
