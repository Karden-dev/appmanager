...
class ChatRepository {
  ...
  Future<void> replaceTempMessage(int tempId, Message realMessage) async {
    ...
    try {
      await db.transaction((txn) async {
        // 1. Supprimer l'ancien message temporaire
        await txn.delete(
          DatabaseService.tableMessages,
          where: 'id = ?',
          whereArgs: [tempId],
        );
        // 2. Insérer le nouveau message réel
        await txn.insert(
          DatabaseService.tableMessages,
      ...
      // En cas d'échec, on insère quand même le message réel
  ...
  Future<List<Message>> getCachedMessages(
      int orderId, int currentUserId) async {
    final db = await _dbService.database;
    try {
      final List<Map<String, dynamic>> maps = await db.query(
        DatabaseService.tableMessages,
