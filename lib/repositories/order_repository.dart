...
class OrderRepository {
  ...
  Future<List<AdminOrder>> _getOrdersFromDb({
    ...
    if (startDate != null) {
      ...
      whereArgs.add(DateFormat('yyyy-MM-ddT00:00:00').format(startDate));
    }
    if (endDate != null) {
      whereClauses.add("created_at <= ?");
      whereArgs.add(DateFormat('yyyy-MM-ddT23:59:59').format(endDate));
    }
  ...
  Map<String, dynamic> _orderToMap(AdminOrder order) {
    return {
      'id': order.id,
      // *** CORRECTION : Ce champ existe maintenant dans le modèle ***
  ...
  AdminOrder _orderFromMap(Map<String, dynamic> map, List<OrderItem> items, List<OrderHistoryItem> history) {
    return AdminOrder(
      id: map['id'] as int,
      // *** CORRECTION : Ce paramètre existe maintenant dans le constructeur ***
