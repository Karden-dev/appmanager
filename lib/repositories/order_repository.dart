...
class OrderRepository {
  ...
  Future<void> saveOrder(Map<String, dynamic> orderData, int? orderId) async {
    ...
    // 2. --- CORRECTION CRITIQUE : Préparer le payload pour l'API ---
    final payloadData = {
  ...
  Future<void> _syncOrdersToDb(List<AdminOrder> apiOrders, {bool clearExisting = true}) async {
    ...
    await db.transaction((txn) async {
      ...
      for (final order in apiOrders) {
        ...
        // Supprimer les anciens items/history pour cette commande
  ...
  /// Récupère les commandes de la DB (avec jointures)
  Future<List<AdminOrder>> _getOrdersFromDb({int? orderId, List<String>? statuses}) async {
    ...
    // 2. Récupérer tous les items et history
    ...
    // 3. Mapper les items et history par order_id
    ...
    // 4. Construire les objets AdminOrder
  ...
  // --- MAPPERS (MÉTHODES QUI MANQUAIENT) ---
