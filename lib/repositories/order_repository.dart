...
class OrderRepository {
  ...
  Future<List<AdminOrder>> fetchAdminOrders({
    ...
    // Charge depuis la BDD locale, en appliquant les filtres localement
  ...
  Future<List<ReturnTracking>> fetchPendingReturns({
    ...
    // Note: Le deliverymanId n'est pas stocké dans la table return_tracking de la v3
  ...
  Future<void> updateOrderStatus(
      ...
      // NOUVEAU : Enregistrement de la date de suivi dans la DB locale
      ...
      final payload = {
        ...
        // NOUVEAU : Ajout de la date de suivi pour la synchronisation API
        'followUpAt': followUpAt?.toIso8601String(),
      };
      
      // Nettoyage des valeurs nulles pour le payload (bonne pratique)
  ...
  Future<List<AdminOrder>> _getOrdersFromDb({
    ...
    // *** MODIFICATION : Filtre créé OU suivi pour correspondre à la webapp ***
    if (startDate != null && endDate != null) {
      ...
      // (created_at BETWEEN start AND end) OR (follow_up_at BETWEEN start AND end)
      whereClauses.add(
  ...
  // *** CORRECTION : Utilisation de la méthode de construction programmatique pour _orderToMap ***
  Map<String, dynamic> _orderToMap(AdminOrder order) {
    // Initialiser la carte avec les valeurs non-nulles
    ...
    // Ajouter les valeurs nullables (String?, int?, double?)
    // C'est cette méthode qui empêche l'analyseur de faire une mauvaise inférence
    ...
    map['follow_up_at'] = order.followUpAt?.toIso8601String(); // L'ancienne ligne 387
  ...
  AdminOrder _orderFromMap(Map<String, dynamic> map, List<OrderItem> items, List<OrderHistoryItem> history) {
    return AdminOrder(
      ...
      // (Les lints de Sévérité 2 sont ici, mais le code est correct et sûr)
  ...
  // *** CORRECTION : Utilisation de la méthode de construction programmatique pour _returnToMap ***
