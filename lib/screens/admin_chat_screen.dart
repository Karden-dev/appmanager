...
class _AdminChatScreenState extends State<AdminChatScreen> {
  ...
  // --- CORRECTION : Logique de scroll améliorée (issue de karden-dev) ---
  void _scrollToBottom({bool jump = false}) {
    // On attend que le frame soit complètement rendu AVANT de calculer le maxScrollExtent
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _sendMessage() {
    ...
    // L'envoi est optimiste (n'attend pas l'API)
    context.read<ChatProvider>().sendMessage(content);

    _messageController.clear();
    FocusScope.of(context).unfocus();
    
    // Le Selector s'occupera du scroll en appelant _scrollToBottom
  }
  ...
  Widget build(BuildContext context) {
    ...
    final messagesList = Selector<
      ...
      // --- CORRECTION : Appel direct à _scrollToBottom ---
      builder: (context, data, _) {
        if (data.messages.isNotEmpty) {
          // On appelle _scrollToBottom() directement.
          // La fonction elle-même contient le 'addPostFrameCallback'.
          _scrollToBottom();
        }
