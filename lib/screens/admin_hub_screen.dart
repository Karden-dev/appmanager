...
import 'package:flutter/foundation.dart'; // <-- AJOUT pour listEquals
...
class _AdminHubScreenState extends State<AdminHubScreen> with TickerProviderStateMixin {
  ...
  Future<void> _markOrderAsReady(int orderId) async {
    ...
    // --- MODIFICATION (Confirmée) : Le try...catch gère l'erreur réseau ---
    try {
      ...
      if (!mounted) return;
      // Ce SnackBar affichera l'erreur "Offline"
  ...
  Widget _buildPreparationList(BuildContext context) {
    return Selector<OrderProvider,
        ({List<AdminOrder> orders, bool isLoading})>(
      selector: (_, provider) => (
        orders: provider.hubPreparationOrders,
        isLoading: provider.isLoading
      ),
      shouldRebuild: (prev, next) =>
          prev.isLoading != next.isLoading ||
          !listEquals(prev.orders, next.orders),
      builder: (context, data, child) {
        final hubOrders = data.orders;

        if (data.isLoading && hubOrders.isEmpty) {
          return const Center(child: CircularProgressIndicator());
        }

        if (hubOrders.isEmpty) {
          return const Center(
              child: Text('Aucune commande en attente de préparation.'));
        }
        ...
        if (filteredOrders.isEmpty) {
          return const Center(
              child: Text('Aucun résultat ne correspond à la recherche.'));
        }

        // 1. Grouper par Livreur (ID)
        final Map<int?, List<AdminOrder>> groupedOrders =
            filteredOrders.fold({}, (map, order) {
        ...
        final sortedGroups = groupedOrders.entries.toList()
          ..sort((a, b) {
            if (a.key == -1) return -1;
            if (b.key == -1) return 1;
            return (a.value.first.deliverymanName ?? '')
                .compareTo(b.value.first.deliverymanName ?? '');
          });
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final groupName = group.key == -1
                ? 'Non Assigné'
                : group.value.first.deliverymanName ?? 'Livreur Inconnu';
            final readyCount = group.value
                .where((o) => o.status == 'ready_for_pickup')
                .length;
            final inProgressCount =
                group.value.where((o) => o.status == 'in_progress').length;
            ...
            return Column(
              ...
              children: [
                ...
                ...group.value.map((order) => HubOrderCard(
                      order: order,
                      onMarkAsReady: () =>
                          _markOrderAsReady(order.id), // Utilisez l'ID de la commande
                      onRefresh: _fetchPreparationOrders,
                    )),
              ],
            );
      ...
      children: [
        ...
        Expanded(
          child: Selector<OrderProvider,
              ({List<ReturnTracking> returns, bool isLoading})>(
            selector: (_, provider) => (
              returns: provider.pendingReturns,
              isLoading: provider.isLoading
            ),
            shouldRebuild: (prev, next) =>
                prev.isLoading != next.isLoading ||
                !listEquals(prev.returns, next.returns),
            builder: (context, data, child) {
              final returns = data.returns;

              if (data.isLoading && returns.isEmpty) {
                return const Center(child: CircularProgressIndicator());
              }
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ReturnCard(
                    returnItem: item,
                    onConfirmReception: () =>
                        _markReturnAsCompleted(item.trackingId),
                    onRefresh: _fetchPendingReturns,
                  );
                },
              );
