// lib/screens/admin_order_edit_screen.dart
...
class OrderItemEditor extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Conteneur pour séparer visuellement chaque article
    return Container(
          ...
          Expanded(
            ...
            child: TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Article',
                // Laisse le thème global définir la bordure
...
class _AdminOrderEditScreenState extends State<AdminOrderEditScreen> {
  ...
  void initState() {
    ...
    if (_isEditMode) {
      ...
      // CORRECTION (Erreurs 1, 2): Utilisation de order.shop (objet imbriqué)
      _selectedShop = order.shop; 
      _shopSearchController.text = order.shop.name; 
      
      // CORRECTION (Erreur 3): order.customerName -> order.clientName
      _customerNameController.text = order.clientName;
      // CORRECTION (Erreur 4): order.customerPhone -> order.clientPhone
      _customerPhoneController.text = order.clientPhone;
      // CORRECTION (Erreur 5): order.deliveryLocation -> order.clientAddress
      _deliveryLocationController.text = order.clientAddress;
      
      _deliveryFeeController.text = order.deliveryFee.toStringAsFixed(0);
      
      // CORRECTION (Erreurs 6, 7): 'expeditionFee' est nullable (double?)
      // Ajout d'une vérification (?? 0) avant d'appeler .toStringAsFixed() ou de comparer.
      _expeditionFeeController.text = (order.expeditionFee ?? 0).toStringAsFixed(0);
      _isExpedition = (order.expeditionFee ?? 0) > 0;
  ...
  Future<void> _saveForm() async {
    ...
    try {
      // Capture du contexte avant l'await
      final provider = Provider.of<OrderProvider>(context, listen: false);
      final messenger = ScaffoldMessenger.of(context);
      final navigator = Navigator.of(context);

      await provider.saveOrder(orderData, widget.order?.id);
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : Form(
              key: _formKey,
              child: ListView(
                padding: const EdgeInsets.all(12.0), // Padding général augmenté
                children: [
                      ...
                      Autocomplete<Shop>(
                        fieldViewBuilder: (context, textEditingController, focusNode, onFieldSubmitted) {
                           ...
                           return TextFormField(
                            ...
                            decoration: const InputDecoration(
                              ...
                              // La décoration est gérée par le thème, pas de 'border: OutlineInputBorder()'
                            ),
                            validator: (value) {
                        ...
                        optionsBuilder: (TextEditingValue textEditingValue) async {
                          if (textEditingValue.text.isEmpty) {
                            // Si l'utilisateur efface le champ, désélectionner le marchand
                    ...
                    children: [
                       ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          // Utilisation du OrderItemEditor stylisé
                  ...
                  _buildSectionCard(
                    ...
                    children: [
                      ...
                      // Champ Frais d'Expédition (si c'est une expédition)
                      if (_isExpedition)
                        Padding(
                        ...
                        const SizedBox(height: 16),
                        // Bouton d'action final (sauvegarde)
                        ElevatedButton.icon(
                          onPressed: _isLoading ? null : _saveForm, 
                          icon: const Icon(Icons.save), 
  ...
  // Helper pour créer les cartes de section (Amélioré)
