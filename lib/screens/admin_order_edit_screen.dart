...
class _AdminOrderEditScreenState extends State<AdminOrderEditScreen> {
  ...
  void initState() {
    ...
    if (_isEditMode) {
      ...
      // *** CORRECTION : Assigner le shopId depuis l'objet order ***
      ...
      if (order.items.isNotEmpty) {
        for (var item in order.items) {
         ...
         // Gérer le cas où une commande n'a pas d'articles (si possible)
         _addItemRow();
      }
    } else {
      _addItemRow();
  ...
  Future<void> _saveForm() async {
    ...
    // --- MODIFICATION : Ajout du try...catch pour la logique Online-First ---
    try {
      ...
      // Intercepte l'erreur (offline, erreur serveur...)
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
