...
import 'package:wink_manager/widgets/shop_edit_dialog.dart'; // Import de la modale
import 'package:wink_manager/utils/app_theme.dart'; // Import pour les couleurs

// --- Widget pour gérer la ligne d'un article (Inchangé) ---
...
class _AdminOrderEditScreenState extends State<AdminOrderEditScreen> {
  ...
  void initState() {
    ...
    if (_isEditMode) {
      ...
      _selectedShop = Shop(id: order.shopId, name: order.shopName, phoneNumber: ''); 
  ...
  Future<void> _saveForm() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Erreur lors de la sauvegarde: ${e.toString().replaceFirst('Exception: ', '')}'), backgroundColor: Colors.red),
      );
    } finally {
      if (mounted) {
        setState(() { _isLoading = false; });
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _buildSectionCard(
                    ...
                    children: [
                      // --- CHAMP MARCHAND AVEC BOUTON INTÉGRÉ ---
                      Autocomplete<Shop>(
                        fieldViewBuilder: (context, textEditingController, focusNode, onFieldSubmitted) {
                           return TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              // BOUTON INTÉGRÉ ICI (SUFFIX)
                              suffixIcon: IconButton(
                                icon: const Icon(Icons.add_business, color: AppTheme.primaryColor),
                                tooltip: 'Ajouter un nouveau marchand',
                                onPressed: () async {
                                  await showDialog(
                                    context: context,
                                    builder: (_) => const ShopEditDialog(),
                                  );
                                  // Une fois fermé, l'admin peut taper le nom du nouveau marchand
                                },
                              ),
                            ),
                            validator: (value) {
