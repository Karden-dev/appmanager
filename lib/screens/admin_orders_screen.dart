...
class _AdminOrdersScreenState extends State<AdminOrdersScreen> {
  ...
  Timer? _debounce;
  // bool _isLoading = true; // Géré par le provider maintenant
  ...
  void _onScroll() {
    ...
    // Si on est en bas de la liste, qu'on ne charge pas déjà, et qu'il y a plus de commandes
  ...
  Future<void> _loadOrders() async {
    // Le provider gère _isLoading, pas besoin de setState ici
    try {
      // Appelle loadOrders (qui charge la page 1 par défaut)
  ...
  Widget build(BuildContext context) {
    // Le provider est juste utilisé pour les filtres ici, la liste est gérée par le Selector
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // --- MODIFIÉ : Remplacement par un Selector pour la pagination ---
          Expanded(
            child: Selector<OrderProvider,
              ...
              builder: (context, data, _) {
                ...
                // Affiche l'erreur SEULEMENT si la liste est vide
                if (data.error != null && orders.isEmpty) {
                  return Center(child: Text(data.error!, style: const TextStyle(color: Colors.red)));
                }

                // Affiche "Aucune commande" SEULEMENT si la liste est vide et qu'on ne charge pas
                ...
                return RefreshIndicator(
                  onRefresh: _loadOrders, // Appelle loadOrders (page 1)
                  child: ListView.builder(
                    // --- NOUVEAU : Ajout du controller ---
                    controller: _scrollController,
                    padding: const EdgeInsets.fromLTRB(8.0, 4.0, 8.0, 80.0), // Padding pour le FAB
                    itemCount: itemCount,
                    itemBuilder: (context, index) {
                      ...
                      if (index == orders.length) {
                        ...
                        return const SizedBox.shrink(); // Ne rien afficher à la fin
                      }
                      // --- FIN NOUVEAU ---

                      // Logique d'affichage de la carte (inchangée)
