...
import 'package:wink_manager/models/user.dart'; // Import pour le modèle User
...
class CashService {
  ...
  // --- 7. NOUVEAU : Recherche Utilisateurs (Pour Bénéficiaire Dépense) ---
  // CORRECTION : Utilisation robuste de User.fromJson
  Future<List<User>> searchUsers(String query) async {
    try {
      final response = await _dio.get('/users', queryParameters: {
        'search': query,
        'limit': 20, // Limite raisonnable pour l'autocomplete
      });
      
      List<dynamic> usersList = [];
      
      // Adaptation selon la structure de réponse possible (Tableau direct ou objet paginé)
      if (response.data is List) {
        usersList = response.data;
      } else if (response.data is Map && response.data.containsKey('users')) {
         usersList = response.data['users'];
      } else if (response.data is Map && response.data.containsKey('data')) {
         // Cas pagination Laravel/Express standard
