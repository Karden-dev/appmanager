...
class DatabaseService {
  ...
  static const _databaseVersion = 5; // <-- VERSION MISE À JOUR
  ...
  Future _onCreate(Database db, int version) async {
    // --- MODIFICATION : Ajout de shop_id et follow_up_at ---
    await db.execute('''
  ...
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 4) {
      ...
      try {
        // Cette migration a été effectuée par le passé.
    ...
    // --- NOUVELLE MIGRATION V5 : Ajout de follow_up_at ---
    if (oldVersion < 5) {
      if (kDebugMode) print('DatabaseService: Application de la v5 (Ajout follow_up_at à orders)...');
      try {
        await db.execute('ALTER TABLE $tableOrders ADD COLUMN follow_up_at TEXT');
      } catch (e) {
         if (kDebugMode) print('Erreur migration v5 (colonne existe peut-être déjà): $e');
      }
    }
    // --- FIN MIGRATION V5 ---
  }
