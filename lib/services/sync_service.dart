...
import 'dart:async'; // <-- CORRECTION: 'dart;async' remplacé par 'dart:async'
...
class SyncService {
  ...
  // Le type est maintenant correctement défini grâce à l'import corrigé ci-dessus
  ...
  void initialize() {
    ...
    // La méthode 'checkConnectivity()' retourne une LISTE (v6.x)
    ...
    _connectivitySubscription =
      ...
      _handleConnectivityChange(results); // 'listen' attend une LISTE
    });
  }

  // La méthode accepte une LISTE (v6.x)
  void _handleConnectivityChange(List<ConnectivityResult> results) {
    if (kDebugMode) {
      print('SyncService: Statut réseau changé. Online: $_isOnline');
    }

    // Logique pour une LISTE (v6.x)
    // Si la liste ne contient qu'un seul élément ET que c'est 'none', on est offline
