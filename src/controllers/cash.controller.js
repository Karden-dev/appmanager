...
const updateRemittance = async (req, res) => {
    try {
        const { id } = req.params;
        let { amount } = req.body; // Utilisation de let pour la modification
        if (!amount) {
            return res.status(400).json({ message: "Le montant est requis." });
        }
        
        // Assurer que le montant de versement (entrée de caisse) est POSITIF
...
const confirmRemittance = async (req, res) => {
    try {
        ...
        // 1. Validation de la présence des champs obligatoires (non-numériques)
        if (!transactionIds || !Array.isArray(transactionIds) || transactionIds.length === 0 || !validated_by) {
            return res.status(400).json({ message: "Les champs transactionIds et validated_by sont requis." });
        }

        // 2. VÉRIFICATION ET CONVERSION DU PAIDAMOUNT (CORRECTION DÉFINITIVE)
        const paidAmountString = String(paidAmount).trim();

        if (paidAmount === undefined || paidAmount === null || paidAmountString === '' || isNaN(parseFloat(paidAmountString))) {
            return res.status(400).json({ message: "Le montant 'paidAmount' doit être un nombre valide et est requis." });
        }
        
        const numericPaidAmount = parseFloat(paidAmountString);

        // 3. Vérification de la validité du montant numérique
        if (numericPaidAmount < 0) {
            return res.status(400).json({ message: "Le montant versé ne peut pas être négatif." });
        }
        
        // CORRECTION FINALE: S'assurer que les IDs sont traités comme des NOMBRES (sécurité contre l'encodage string)
        const numericTransactionIds = transactionIds.map(id => Number(id));

        // Appel au modèle avec le montant converti en nombre et les IDs convertis
